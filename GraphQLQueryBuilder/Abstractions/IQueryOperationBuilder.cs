...
namespace GraphQLQueryBuilder.Abstractions
{
    public interface IQueryOperationBuilder<T> where T : class
    {
        ...
        IQueryOperationBuilder<T> AddObjectCollectionField<TProperty>(string alias, Expression<Func<T, IEnumerable<TProperty>>> expression, IArgumentCollection arguments, ISelectionSet<TProperty> selectionSet) where TProperty : class;
